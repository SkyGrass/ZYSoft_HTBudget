...
<head runat="server">
	...
	<link href="./modalTree.css" rel="stylesheet" />
...
<body>
				...
				<el-tree ref='tree' :data="clsData" :default-expanded-keys="defaultExpandedKeys" :expand-on-click-node=false :props="defaultProps" node-key="FItemID">
					<span class="custom-tree-node" slot-scope="node">
						...
						<el-row style="margin-left: 20px;" :gutter=5>
							...
							node.data,'FBudgetQty')" @blur="onBlur($event,node.data,'FBudgetQty')" :precision="2" :id="'qty_'+node.data.FItemID" class="txt number_noboard" :value="node.data.FBudgetQty" placeholder="数量" :min="0" />
								</div>
							</el-col>
							<el-col :span=8>
							...
							node.data,'FBudgetPrice')" @blur="onBlur($event,node.data,'FBudgetPrice')" :precision="2" :id="'price_'+node.data.FItemID" class="txt number_noboard" :value="node.data.FBudgetPrice" placeholder="单价" :min="0" />
								</div>
							</el-col>
							<el-col :span=8>
								<div class='contr'><label>金额:</label>
									<input :disabled="query.state == 'read'" type='number' @focus='onFocus' @change="onChange($event,
							node.data,'FBudgetSum')" readonly @blur="onBlur($event,node.data,'FBudgetSum')" :precision="2" :id="'sum_'+node.data.FItemID" class="txt number_noboard" :value="node.data.FBudgetSum" placeholder=" 总价" :min="0" />
								</div>
							</el-col>
						</el-row>
					</span>
	...
	<script src="./modalTree.js"></script>
</body>
