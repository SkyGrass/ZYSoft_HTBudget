<%@ Page Language="C#" AutoEventWireup="true" %>
	...
	<body>
							...
							<el-form ref="form" :rules="rules" :model="form" label-width="100px" size="mini" inline>
								<table>
										...
										<td>
											<el-form-item label="竣工时间" class="form-item-max" prop='endDate'>
												...
												<el-input :readonly="query.state =='read'" clearable style="width:100%"
													v-model="form.endDate" placeholder="请填写竣工日期" class="noBorder">
												</el-input>
											</el-form-item>
										</td>
									</tr>
									<tr>
										...
										<td>
											<el-form-item label="预算金额" class="form-item-max moneyright"
												prop='budgetSum'>
												<el-input-number :disabled="query.state =='read'" style="width:100%"
													:controls=false v-model="form.budgetSum" :min=0 :precision="2"
									...
									<tr>
										...
										<td>
											<el-form-item label="净成本" class="form-item-max moneyright" prop='sum1'>
												<el-input-number :disabled="query.state =='read'" style="width:100%"
													:controls=false v-model="form.sum1" :min=0 :precision="2"
									...
									<tr>
										<td>
											<el-form-item label="项目类型" class="form-item-max1" prop='projectType'>
												<el-input :readonly="query.state =='read'" clearable style="width:100%"
													v-model="form.projectType" placeholder="请填写项目类型" class="noBorder">
												</el-input>
											</el-form-item>
										</td>
										<td>
											<el-form-item label="预算与实际成本差异额" class="form-item-max moneyright">
												<span slot='label'>预算与实际成本差异额</span>
												<el-input-number style="width:100%;" disabled :controls=false
													v-model="form.totalSum" :precision="2" placeholder="请填写预算与实际成本差异额"
							...
							<table border='1' style="text-align: center">
								<thead>
									<tr style="background-color: #e6e6e6;">
										...
										<th style="padding:8px;width: 30%;">名称</th>
										<th style="padding:8px">预算金额</th>
										<th style="padding:8px">成本金额</th>
										<th style="padding:8px">差异金额</th>
										<th style="padding:8px">差异率</th>
									</tr>
								</thead>
								<tr :class="t.FGroupCode == activeName ? 'hightRow' : '' " v-for='t in summary'
									...
									<td style="width:150px;padding:10px">{{t.FDiffSum}}</td>
									<td style="width:150px;padding:10px">{{t.FDiffRate}}</td>
								</tr>
							</table>
						</el-col>
					</el-row>
								...
								<table border='1' style="text-align: center">
									<thead>
										<tr style="background-color: #e6e6e6;">
											...
											<th style="padding:8px">预算数量</th>
											<th style="padding:8px">成本数量</th>
											<th style="padding:8px">差异数量</th>
											<th style="padding:8px">预算金额</th>
											<th style="padding:8px">净成本</th>
											<th style="padding:8px">差异金额</th>
											<th style="padding:8px">差异率</th>
										</tr>
									</thead>
									<tr class="tr" v-for='t in tab.FChildren' :key='t.FItemCode'>
										...
										<td style="width:150px;padding:10px">{{t.FDiffQty}}</td>
										<td style="width:150px;padding:10px">{{t.FBudgetSum}}</td>
										<td style="width:150px;padding:10px">{{t.FCostSum}}</td>
										<td style="width:150px;padding:10px">{{t.FDiffSum}}</td>
										<td style="width:150px;padding:10px">{{t.FDiffRate}}</td>
		...
		<script src="./js/modules/diffForm.js"></script>
	</body>
